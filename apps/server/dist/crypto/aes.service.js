 ...
 */
function decryptPayload(encryptedData, ivBase64, authTagBase64, aesKeyBuffer) {
    try {
        const iv = Buffer.from(ivBase64, 'base64');
        const authTag = Buffer.from(authTagBase64, 'base64');
        const ciphertext = Buffer.from(encryptedData, 'base64');
        const decipher = crypto_1.default.createDecipheriv(ALGORITHM, aesKeyBuffer, iv);
        decipher.setAuthTag(authTag);
        const decrypted = Buffer.concat([decipher.update(ciphertext), decipher.final()]);
        return decrypted.toString('utf8');
    }
    catch (error) {
        throw new apiError_1.ApiError(400, 'Failed to decrypt AES payload');
    }
}
/**
 ...
 */
function encryptPayload(data, aesKeyBuffer) {
    try {
        const iv = crypto_1.default.randomBytes(12);
        const cipher = crypto_1.default.createCipheriv(ALGORITHM, aesKeyBuffer, iv);
        ...
        throw new apiError_1.ApiError(500, 'Failed to encrypt AES payload');
    }
}
