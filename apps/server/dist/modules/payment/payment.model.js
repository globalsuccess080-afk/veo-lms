...
const paymentSchema = new mongoose_1.Schema({
    ...
    status: { type: String, enum: enums_1.PAYMENT_STATUSES, default: 'created' },
    metadata: {
        courseName: String,
        coursePrice: Number
    },
...
paymentSchema.pre('save', function (next) {
    if (this.isModified('razorpaySignature') && this.razorpaySignature && !this.razorpaySignature.startsWith('enc:')) {
        this.razorpaySignature = 'enc:' + (0, encryption_1.encrypt)(this.razorpaySignature);
    }
    next();
});
paymentSchema.methods.getDecryptedSignature = function () {
    if (!this.razorpaySignature)
        return null;
    if (this.razorpaySignature.startsWith('enc:'))
        return (0, encryption_1.decrypt)(this.razorpaySignature.slice(4));
